...
const resolveServiceCommands = async (composeData, docker, log, engine, composeFilePaths, project, orchestratorSeperator) => {
  ...
  for (const data of composeData) {
    ...
    for (const [serviceName, service] of Object.entries(services)) {
      ...
      try {
        const info = await inspectImage(docker, service.image ?? project + orchestratorSeperator + serviceName);
        ...
        try {
          ...
          log.error('Failed to build/pull compose docker images, continuing without entrypoint override...');
          console.log(e);
        }
      }
    }
...
module.exports = async (files, dir, landoComposeConfigDir, engine, project, envFiles, log, orchestratorSeperator) => {
  ...
  return resolveServiceCommands([result], engine.docker, log, engine, composeFilePaths, project, orchestratorSeperator);
};
