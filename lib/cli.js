...
module.exports = class Cli {
  ...
  /**
   * Returns the parsed global options that belong to lando itself
   *
   * For passthrough commands eg tooling, everything after the command name belongs to that command and not to
   * us. Without this `lando console my:command -vvv` would crank up landos verbosity when the user only wanted
   * the underlying command to be verbose.
   *
   * @since 3.26.5
   * @alias lando.cli.landoArgv
   * @return {Object} Yarg parsed options
   */
  landoArgv() {
    // reuse the last result if nothing has changed, defaultConfig alone asks for this twice
    const key = `${this.passthrough}::${process.argv.join(' ')}`;
    if (this._landoArgv && this._landoArgvFor === key) return this._landoArgv;

    // if this is not a passthrough command then all global options are ours
    if (!this.passthrough) return this.argv();

    // otherwise only consider the args that come before the command
    const args = process.argv.slice(2);
    const index = args.indexOf(this.passthrough);
    if (index === -1) return this.argv();

    // note that we need a fresh yargs here because the singleton has demandCommand and middleware on it by now.
    // we also drop "choices" because the validation for those is handled manually in run() and we do not want
    // a bad value to make yargs bail out from in here
    const parser = require('yargs/yargs')(args.slice(0, index)).help(false).version(false);
    _.forEach(globalOptions, (config, name) => parser.option(name, _.omit(config, ['choices'])));

    this._landoArgvFor = key;
    this._landoArgv = parser.parse();
    return this._landoArgv;
  }
  ...
  defaultConfig(appConfig = {}) {
    ...
    const config = {
      ...
      logLevelConsole: (this.landoArgv().verbose) ? this.landoArgv().verbose + 1 : this.logLevel,
  ...
  isDebug() {
    const {debug, verbose} = this.landoArgv();
   ...
   */
  // note that "yes" stays on argv() because a "-y" after a command is still meant for us
  handleError(error, handler, verbose = this.landoArgv().verbose, lando = {}, yes = this.argv().yes) {
    // Set the verbosity
  ...
  parseToYargs({
    ...
    const handler = argv => {
      // if this command passes its args through then note that so we only claim global options that show up
      // before it, this needs to happen before anything reads our config or verbosity
      if (passthrough) {
        this.passthrough = command.split(' ')[0];
        // and drop the args that belong to the command so they cannot influence our own option handling
        argv = require('../utils/strip-passthrough-args')(argv, {
          options,
          positionals,
          dynamic,
          globals: this.landoArgv(),
        });
      }
  ...
  run(tasks = [], config = {}) {
    ...
    // work out whether this command passes its args through before we look at any global option, otherwise
    // something like "lando console my:command --clear" would wipe our caches and exit without ever running it
    this.passthrough = require('../utils/get-passthrough-command')(tasks, yargs.argv._[0]);

    const {clear, channel, experimental, secretToggle} = this.landoArgv();
